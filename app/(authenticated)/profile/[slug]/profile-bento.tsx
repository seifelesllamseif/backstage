...
import { ArrowUpRight, Sparkles, Pencil, X, Check } from "lucide-react";
...
// adapted for Backstage. Inline edit — the bento toggles into an edit state
// where the About / Languages / Socials / Avatar cards become inputs inside
// a single form. One save action; no separate edit route.
...
export function ProfileBento({
  ...
  const [state, action, pending] = useActionState<UpdateProfileState, FormData>(
    async (prev, formData) => {
      ...
      // Successful save → exit edit mode. (Error keeps the form open so the
  ...
  // When editing, wrap the bento grid in a <form>. Otherwise it's a plain
  // <div>. We keep the layout identical; only swap text for inputs inside
  // editable cards.
  const gridClass =
  ...
  const innerGrid = (
    <>
        ...
        {editing ? (
          ...
          >
            {(() => {
              ...
              return socials.map(({ key, href, label }) => (
                <a
                  key={key}
                  href={href}
                  target="_blank"
      ...
      {state?.error && (
        <p
          role="alert"
          className="md:col-span-12 rounded-md border border-destructive/40 bg-destructive/10 px-4 py-2 text-sm text-destructive"
        >
          {state.error}
        </p>
      )}
