...
- The slice-1 plan §4 (since deleted; see git history) — a minimal four-table cut sized to ship slice 1.
- `docs/backstage-schema.sql` — the eventual full schema, ~30 tables, dozens of fields per table.

Examples of disagreement: the plan calls for `team_member.avatar_initials` (a tiny string for the UI's circle badge); the SQL has `avatar_url`. The plan's `team_member` has 7 columns; the SQL's has ~14. Modeling everything now means slice 1 ships with unused columns and the plan/code drift further.
...
Concretely:
- Only the four slice-1 tables exist in Prisma: `company`, `team_member`, `project`, `task`.
...
| `team_member` | `id`, `company_id`, `email`, `full_name`, `avatar_initials`, `access_tier`, `created_at`. Unique `(company_id, email)`. |
...
### FK behavior (matches `backstage-schema.sql`)

| FK | On delete |
|---|---|
| `team_member.company_id` → `company.id` | cascade |
| `project.company_id` → `company.id` | cascade |
| `task.company_id` → `company.id` | cascade |
| `task.project_id` → `project.id` | cascade |
| `task.assignee_id` → `team_member.id` | set null |
| `task.created_by` → `team_member.id` | set null |
| `team_member.id` → `auth.users.id` | cascade (hand-written; see [0002](0002-supabase-prisma-boundary.md)) |
...
- `gen_random_uuid()` (pgcrypto) is used as the default for `company.id`, `project.id`, `task.id` — matches the SQL schema. `team_member.id` has **no default**: its value comes from Supabase Auth per [0002](0002-supabase-prisma-boundary.md).
