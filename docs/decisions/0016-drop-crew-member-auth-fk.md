...
# 0016 — Drop the `crew_member.id → auth.users.id` foreign key

## Context

[Decision 0002](0002-supabase-prisma-boundary.md) established the Prisma ↔ Supabase boundary: Supabase owns the `auth` schema; Prisma owns `public`. To keep `crew_member.id` tied to `auth.users.id` we wrote a hand-managed cross-schema FK in `prisma/sql/crew_member_auth_fk.sql`, applied via the Supabase MCP after `prisma migrate`.
...
The FK was load-bearing for one behavior only: cascade-delete of `crew_member` when an `auth.users` row is deleted. In practice we never delete auth users in slice 1 or 2 (the seed creates them; nothing removes them). The cost of keeping the FK across migrations outweighed the value.
...
- The Postgres FK `crew_member_auth_user_fk` was dropped via the Supabase MCP (`alter table public.crew_member drop constraint crew_member_auth_user_fk`).
- `prisma/sql/crew_member_auth_fk.sql` is **no longer applied** to new databases. Kept in the repo for the historical record and in case a future slice wants to re-introduce it through a different mechanism.

### What stays the same

The boundary from [0002](0002-supabase-prisma-boundary.md) is otherwise intact:

- **`crew_member.id` still equals an `auth.users.id`.** The seed and any future signup flow create the auth user first, then insert the `crew_member` with that id. This is an application-level invariant now, not a database-level one.
- Prisma still owns `public`; Supabase still owns `auth`. They just don't touch each other through a constraint.

### Trade-offs we accept

- **No cascade-delete.** Deleting an `auth.users` row leaves an orphan `crew_member` row referencing a missing user. Today this is a hypothetical because we never delete auth users; if we add a "remove a teammate" flow later, the server action does both deletes in order (auth admin API → Prisma delete) and we're done.
- **No referential integrity at the database level.** An incorrect application path could insert a `crew_member` with a non-existent `id`. Mitigation: all writes go through `lib/dal.ts` / server actions that derive the id from a verified auth session — the path can't realistically produce a bogus value.
...
- **The seed continues to assign `crew_member.id = auth.users.id` by application logic.** No code change to `prisma/seed.ts` was needed for this drop.
