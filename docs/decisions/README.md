...
- [0002 — Supabase / Prisma boundary](0002-supabase-prisma-boundary.md) — the `auth.users` vs `team_member` rule.
...
- [0009 — Seed script approach](0009-seed-script-approach.md) — one-shot, shared dev password, VERB-themed; needs the service role key.
...
- [0014 — Step 7 polish scope](0014-step-7-polish-scope.md) — Verbivore tokens wired into Tailwind v4 (single-mode dark), generic loading.tsx + error.tsx in the authenticated group, design fidelity to /backstage deferred.
...
- [0016 — Drop the `team_member → auth.users` FK](0016-drop-team-member-auth-fk.md) — the cross-schema FK from 0002 conflicted with Prisma 7's drift detector; the boundary stays, the constraint goes (supersedes parts of 0002 and 0006).
...
- [0018 — Profile pages (bento port)](0018-profile-pages.md) — every team_member gets `/profile/[slug]`, `/profile` redirects to own slug, six new columns on team_member (bio/avatarUrl/socials/languages), framer-motion fade-in, inline edit on the bento.
...
- [0031 — Portfolio at /[slug]](0031-portfolio-shareable-slug-url.md) — root-level shareable URLs; single public-shaped view for everyone (owner uses /dashboard for live work); supabase-js + cookie-bound client; same-company access only.
